		...
		if (tableName && dataToInsert) {
			if (Object.size(dataToInsert) > 0) {
				...
				for (index in dataToInsert) {
					if (typeof(dataToInsert[index]) == 'string') {
						if (/^`.*`$/.test(dataToInsert[index])) {
							mysqlQuery += dataToInsert[index].replace(/"/g, '\\"').replaceAll('`','');
						} else {
							mysqlQuery += '"' + dataToInsert[index].replace(/"/g, '\\"') + '"';
						}
					} else {
						mysqlQuery += dataToInsert[index];
					}
					mysqlQuery += (i + 1 == Object.size(dataToInsert) ? ') ' : ', ');
					i++;
				}
			...
			if (Object.size(data) > 0) {
				...
				for (var index in data) {
					...
					if (typeof(data[index]) == 'string') {
						if (/^`.*`$/.test(data[index].value)) {
							mysqlQuery += data[index].replace(/"/g, '\\"').replaceAll('`','');
						} else {
							mysqlQuery += '"' + data[index].replace(/"/g, '\\"') + '"';
						}
					} else {
						mysqlQuery += data[index];
					}
				...
				if (conditions) {
					...
					for (index in conditions) {
						...
						mysqlQuery += " ";
						if (typeof(conditions[index].value) == 'string') {
							if (/^`.*`$/.test(conditions[index].value)) {
								mysqlQuery += conditions[index].value.replace(/"/g, '\\"').replaceAll('`','');
							} else {
								mysqlQuery += '"' + conditions[index].value.replace(/"/g, '\\"') + '"';
							}
						} else {
							mysqlQuery += conditions[index].value;
						}
